...
void random_int_uniform::ran_array (int32_t* aa, size_t n) {
    size_t i, j;
    for (j = 0; j < KK; j++) aa[j] = _ran_x[j];
    for (; j < n; j++) aa[j] = _mod_diff(aa[j-KK], aa[j-LL]);
    for (i = 0; i < LL; i++, j++) _ran_x[i] = _mod_diff(aa[j-KK], aa[j-LL]);
    for (; i < KK; i++, j++) _ran_x[i] = _mod_diff(aa[j-KK], _ran_x[i-LL]);
}

void random_int_uniform::ran_start (int seed) {
    int t, j;
...
int random_int_uniform::_ran_arr_cycle () {
    if (_ran_arr_ptr == &_ran_arr_dummy)
...
const int32_t& random_int_uniform::ran_x(size_t i) const {
...
void random_double_uniform::ranf_array (double* aa, size_t n) {
    // aa: destination
...
void random_double_uniform::ranf_start (int32_t seed) {
    int t, s, j;
...
double random_double_uniform::_ranf_arr_cycle () {
    if (_ranf_arr_ptr == &_ranf_arr_dummy)
        ranf_start (314159LL); // the user forgot to initialize
...
const double& random_double_uniform::ran_u(size_t i) const {
